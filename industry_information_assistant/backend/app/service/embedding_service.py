...
def generate_embedding(
    ...
    """
    api_key = api_key or _env_first("EMBEDDING_API_KEY", "LLM_API_KEY", "DASHSCOPE_API_KEY", "OPENAI_API_KEY")
    base_url = base_url or _env_first("EMBEDDING_BASE_URL", "LLM_BASE_URL", "DASHSCOPE_BASE_URL", "OPENAI_BASE_URL")
    model_name = model_name or _env_first("EMBEDDING_MODEL")

    if not api_key:
        print("错误: 缺少 EMBEDDING_API_KEY/LLM_API_KEY/DASHSCOPE_API_KEY 环境变量")
        return None
    if not model_name:
        print("错误: 缺少 EMBEDDING_MODEL 环境变量")
        return None
