...
from ..trace_recorder import CURRENT_TRACE
...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self.role = role
        self.model = model
        # Newer OpenAI clients validate credentials at construction time.
        # Keep agent initialization possible in no-key smoke tests; actual LLM
        # calls will still fail and be handled by each agent's fallback/error path.
        client_kwargs = {"api_key": llm_api_key or "EMPTY_API_KEY"}
        if llm_base_url:
    ...
    def add_message(self, state: ResearchState, event_type: str, content: Any) -> None:
        ...
        trace = CURRENT_TRACE.get()
        if trace:
            trace.record_message(self.name, event_type, message)
