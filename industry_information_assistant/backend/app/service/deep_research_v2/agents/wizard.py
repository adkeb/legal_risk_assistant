...
{{
    ...
    "code": "sns.set_theme(style='whitegrid')\\ndata = {{'Risk': ['高风险', '中风险'], 'Value': [3, 5]}}\\ndf = pd.DataFrame(data)\\nplt.figure(figsize=(12, 7), dpi=200)\\nplt.bar(df['Risk'], df['Value'], color='#6366f1')\\nplt.title('风险等级分布', fontsize=18, fontweight='bold')\\nplt.xlabel('风险等级', fontsize=14)\\nplt.ylabel('数量', fontsize=14)\\nsns.despine()\\nplt.savefig('chart.png', dpi=200, bbox_inches='tight', facecolor='white')",
...
## 任务
生成桑基图的 ECharts 配置，展示证据链、义务流转或风险传导关系。
...
```"""
    ...
    async def process(self, state: ResearchState) -> ResearchState:
        ...
        config = get_config()
        if not getattr(config.research, "enable_code_execution", False):
            state.setdefault("code_executions", []).append({
                "status": "skipped",
                "reason": "法律风控模式默认关闭代码执行。",
            })
            self.add_message(state, "research_step", {
                "step_id": f"step_code_skipped_{uuid.uuid4().hex[:8]}",
                "step_type": "code_execution_skipped",
                "title": "已跳过代码执行",
                "subtitle": "法律风控模式使用模板图表",
                "status": "completed",
                "stats": {"code_executions": len(state.get("code_executions", []))}
            })
            self.add_message(state, "thought", {
                "agent": self.name,
                "content": "法律风控模式默认关闭 LLM 生成代码执行，已使用模板图表。"
            })
            return state
