...
try:
    from service.deep_research_v2.state import ensure_artifact_state_defaults
except ImportError:
    try:
        from app.service.deep_research_v2.state import ensure_artifact_state_defaults
    except ImportError:
...
class CheckpointService:
    ...
    def save_checkpoint(
        ...
        try:
            ...
            # 清理 state 中不可序列化的内容
            clean_state = self._clean_state_for_storage(ensure_artifact_state_defaults(state))
    ...
    def load_checkpoint(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return ensure_artifact_state_defaults(checkpoint.state_json)
    ...
    def load_full_checkpoint(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result.get("state_json"):
                result["state_json"] = ensure_artifact_state_defaults(result["state_json"])
            # 详细日志
    ...
    def _clean_state_for_storage(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in state.items():
            if str(key).startswith("_"):
                continue
            clean[key] = self._make_json_safe(value)
        return clean

    def _make_json_safe(self, value: Any) -> Any:
        """Recursively convert values to JSONB-safe data."""
