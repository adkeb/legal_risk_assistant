...
import { normalizeReferences, usePageTransport } from '@/utils'
...
export default function Index() {
  ...
  const sendChat = useCallback(
    async (target: API.ChatItem, message: string, attachmentIds?: string[]) => {
      ...
      try {
        void attachmentIds
        const selectedSearchModes = deviceState.searchModes as string[]
        const searchModes = selectedSearchModes.length > 0 ? selectedSearchModes : ['web']
        ...
        try {
          ...
          if (target.type === ChatType.Deepsearch) {
            ...
            if (json.type === 'research_complete') {
              ...
              // 设置引用
              target.reference = normalizeReferences(json.references)
      ...
      for (const msg of messages) {
        ...
        if (msg.role === 'assistant') {
          ...
          if (msg.references_data?.references) {
            chatItem.reference = normalizeReferences(msg.references_data.references)
          }
        }
        ...
        if (response?.success && response?.checkpoint) {
          ...
          if (checkpoint.status === 'completed' || checkpoint.status === 'running') {
            ...
            if (stateJson && chat.list.length === 0) {
              ...
              if (refs.length > 0) {
                assistantItem.reference = normalizeReferences(refs)
              }
              ...
              if (lastAssistant) {
                ...
                const refs = uiState?.references || stateJson?.references || []
                if (refs.length > 0) {
                  lastAssistant.reference = normalizeReferences(refs)
                }
