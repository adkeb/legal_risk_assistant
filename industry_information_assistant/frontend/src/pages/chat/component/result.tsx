...
import { extractHttpUrl } from '@/utils'
        ...
        <Section title="相关知识库来源" icon={IconSource} defaultOpen>
          <div className={styles['chat-message-result__source']}>
            {source.knowledge?.map((item, index) => {
              ...
              // 确保链接有效
              const validLink = extractHttpUrl(item.link)
              const body = (
                <>
              ...
              return validLink ? (
                ...
                <div key={`knowledge-${index}-${item.id}`} className={styles.item}>
                  {body}
                </div>
              )
            })}
          ...
          <div className={styles['chat-message-result__source']}>
            {source.web?.map((item, index) => {
              ...
              return validLink ? (
                ...
                <div key={`web-${index}-${item.id}`} className={styles.item}>
                  {body}
                </div>
              )
            })}
    ...
    () => [
      {
        ...
        renderer(token) {
          ...
          const href = extractHttpUrl(target?.link)
          return `<a class="refrence-token" href="${href || 'javascript: void 0'}" target="${href ? '_blank' : '_self'}" rel="noreferrer">[${Number(index) + 1}]</a>`
        },
      },
    ],
    [item, item.reference],
